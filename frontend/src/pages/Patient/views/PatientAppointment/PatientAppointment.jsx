...
const PatientRegister = () => {
    ...
    const fetchDoctorsBySubDepartment = async (subDepartment) => {
        try {
            // 使用 axios 发送 GET 请求
            const response = await axios.get(`http://localhost:8080/api/appointments/doctors/${subDepartment}`);
    ...
    const handleOk = (record) => {
        ...
        if (record && patientId) {
            ...
            // 发送预约信息到后端
            axios.post('http://localhost:8080/api/appointments/create', appointment)
                .then(response => {
